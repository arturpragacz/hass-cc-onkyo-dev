                ...
                if entry_options is None:
                    ...
                    result = self.async_update_reload_and_abort(
                        ...
                        options={
                            ...
                            OPTION_INPUT_SOURCES: sources_store,
                            OPTION_LISTENING_MODES: entry_options[
                                OPTION_LISTENING_MODES
                            ],
                        },
                    )
...
class OnkyoOptionsFlowHandler(OptionsFlow):
    ...
    _data: dict[str, Any] = {}
    _sources: dict[InputSource, str]
    _sound_modes: dict[ListeningMode, str]

    def __init__(self, config_entry: ConfigEntry) -> None:
        """Initialize options flow."""
        sources_store: dict[str, str] = config_entry.options[OPTION_INPUT_SOURCES]
        self._sources = {InputSource(k): v for k, v in sources_store.items()}
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            ...
            self._sound_modes = {}
            for sound_mode_meaning in user_input[OPTION_LISTENING_MODES]:
                sound_mode = ListeningMode.from_meaning(sound_mode_meaning)
                sound_mode_name = entry_options[OPTION_LISTENING_MODES].get(
                    sound_mode.value, sound_mode_meaning
                )
                self._sound_modes[sound_mode] = sound_mode_name
            ...
            return await self.async_step_sound_modes()

        schema_dict: dict[Any, Selector] = {}

        schema_dict[vol.Required(OPTION_MAX_VOLUME)] = NumberSelector(
            NumberSelectorConfig(min=1, max=100, mode=NumberSelectorMode.BOX)
        )

        for source in self._sources:
            schema_dict[vol.Required(source.value_meaning)] = TextSelector()

        schema_dict[vol.Required(OPTION_LISTENING_MODES)] = SelectSelector(
            SelectSelectorConfig(
        ...
        if suggested_values is None:
            ...
            for source, source_name in self._sources.items():
                suggested_values[source.value_meaning] = source_name
    ...
    async def async_step_sound_modes(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Configure the sound modes' names."""
        if user_input is not None:
            sound_modes_store: dict[str, str] = {}
            for sound_mode_meaning, sound_mode_name in user_input.items():
                sound_mode = ListeningMode.from_meaning(sound_mode_meaning)
                sound_modes_store[sound_mode.value] = sound_mode_name

            return self.async_create_entry(
                data={
                    **self._data,
                    OPTION_LISTENING_MODES: sound_modes_store,
                }
            )

        schema_dict: dict[Any, Selector] = {}

        for sound_mode, sound_mode_name in self._sound_modes.items():
            schema_dict[
                vol.Required(sound_mode.value_meaning, default=sound_mode_name)
            ] = TextSelector()
